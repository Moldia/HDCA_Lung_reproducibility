...
load("epi_163k_org_data_with.velo.RData")
#=============================================
...
save.image("epi_batch_integrated.RData")

load("epi_batch_integrated.RData")
...
# Set the number of principle components for downstream analysis
pcs=50
...
FeaturePlot(lung, reduction = 'umap', features = "COL1A2", pt.size = 3, order = TRUE)
FeaturePlot(lung, reduction = 'umap', features = "EPCAM", pt.size = 3, order = FALSE)
FeaturePlot(lung, reduction = 'umap', features = "HBA1", pt.size = 3, order = FALSE)

#====================================================================
# remove the cells with HBA1 > 4 because they presumably are doublets with red blood cells
lung <- subset(lung, subset = HBA1 < 4)

# # remove the artifact clusters with possible doublets of mesenchymal cells
lung <- subset(lung, idents= c(0:13, 15:16, 18:19) )
DefaultAssay(lung) <-"integrated"

lung #11215 cells
save(lung, file = "epi_batch_integrated_filtered_org.RData")

#====================================================================
#====================================================================
load("epi_batch_integrated_filtered_org.RData")
...
# after filtering of the cells we run pca, again 
DefaultAssay(lung) <- "integrated"
...
lung <- FindClusters(lung, resolution = 0.89)
# run tSNE
...
DimPlot(lung, reduction = "umap2d", label = TRUE, pt.size = 2)
...
pdf("epi_163k_umap3d_clusters.pdf", width = 11.69, height = 8.27, paper = "special")
...
pdf(paste0("epi_163k_umap3d_",metadata_to_plot,".pdf"), width = 11.69, height = 8.27, paper = "special")
...
pdf("epi_cells_umap2d_clusters.pdf",width=11.69,height=8.27,paper="special")
epi_cells_umap2d_clusters <- DimPlot(lung, reduction = "umap2d", cols =  "alphabet2", label = TRUE, pt.size = 3)
plot(epi_cells_umap2d_clusters)
rm(epi_cells_umap2d_clusters)
dev.off()

pdf("epi_cells_umap2d_age.pdf",width=11.69,height=8.27,paper="special")
epi_cells_umap2d_age <- DimPlot(lung, reduction = "umap2d", cols =  "alphabet2", shuffle = T, group.by = "age", pt.size = 3)
plot(epi_cells_umap2d_age)
rm(epi_cells_umap2d_age)
dev.off()

pdf("epi_cells_umap2d_SampleName.pdf",width=11.69,height=8.27,paper="special")
epi_cells_umap2d_SampleName <- DimPlot(lung, reduction = "umap2d", cols =  "alphabet2", group.by = "SampleName", pt.size = 3)
plot(epi_cells_umap2d_SampleName)
rm(epi_cells_umap2d_SampleName)
dev.off()

pdf("epi_cells_umap2d_donor.pdf",width=11.69,height=8.27,paper="special")
epi_cells_umap2d_donor <- DimPlot(lung, reduction = "umap2d", cols =  "alphabet2", group.by = "donor", pt.size = 3)
plot(epi_cells_umap2d_donor)
rm(epi_cells_umap2d_donor)
dev.off()

pdf("epi_cells_umap2d_chemistry.pdf",width=11.69,height=8.27,paper="special")
epi_cells_umap2d_chemistry <- DimPlot(lung, reduction = "umap2d", group.by = "chemistry", pt.size = 3)
plot(epi_cells_umap2d_chemistry)
rm(epi_cells_umap2d_chemistry)
dev.off()

pdf("epi_cells_umap2d_features.pdf",width=11.69,height=8.27,paper="special")
epi_cells_umap2d_features <- FeaturePlot(lung, reduction = "umap2d",  features = "nFeature_RNA", pt.size = 3)
plot(epi_cells_umap2d_features)
rm(epi_cells_umap2d_features)
dev.off()

pdf("epi_cells_umap2d_percent_mt.pdf",width=11.69,height=8.27,paper="special")
epi_cells_umap2d_percent_mt <- FeaturePlot(lung, reduction = "umap2d",  features = "percent.MT", pt.size = 3)
plot(epi_cells_umap2d_percent_mt)
rm(epi_cells_umap2d_percent_mt)
dev.off()

pdf("epi_cells_tsne_clusters.pdf",width=11.69,height=8.27,paper="special")
...
pdf("epi_cells_tsne_age.pdf",width=11.69,height=8.27,paper="special")
epi_cells_tsne_age <- DimPlot(lung, reduction = "tsne", cols =  "alphabet2", shuffle = T, group.by = "age", pt.size = 3)
plot(epi_cells_tsne_age)
rm(epi_cells_tsne_age)
dev.off()

pdf("epi_cells_tsne_SampleName.pdf",width=11.69,height=8.27,paper="special")
epi_cells_tsne_SampleName <- DimPlot(lung, reduction = "tsne", cols =  "alphabet2", group.by = "SampleName", pt.size = 3)
plot(epi_cells_tsne_SampleName)
rm(epi_cells_tsne_SampleName)
dev.off()

pdf("epi_cells_tsne_donor.pdf",width=11.69,height=8.27,paper="special")
epi_cells_tsne_donor <- DimPlot(lung, reduction = "tsne", cols =  "alphabet2", group.by = "donor", pt.size = 3)
plot(epi_cells_tsne_donor)
rm(epi_cells_tsne_donor)
dev.off()

pdf("epi_cells_tsne_chemistry.pdf",width=11.69,height=8.27,paper="special")
epi_cells_tsne_chemistry <- DimPlot(lung, reduction = "tsne", group.by = "chemistry", pt.size = 3)
plot(epi_cells_tsne_chemistry)
rm(epi_cells_tsne_chemistry)
dev.off()

pdf("epi_cells_tsne_percent_mt.pdf",width=11.69,height=8.27,paper="special")
epi_cells_tsne_percent_mt <- FeaturePlot(lung, reduction = "tsne", features = "percent.MT", pt.size = 3)
plot(epi_cells_tsne_percent_mt)
rm(epi_cells_tsne_percent_mt)
dev.off()

pdf("epi_cells_tsne_features.pdf",width=11.69,height=8.27,paper="special")
epi_cells_tsne_features <- FeaturePlot(lung, reduction = "tsne", features = "nFeature_RNA", pt.size = 3)
plot(epi_cells_tsne_features)
rm(epi_cells_tsne_features)
dev.off()
...
for (i in 0:(nlevels(lung@active.ident)-1)) {
  ...
  name = sprintf("epi_umap2d_cluster%s.tiff", i)
...
for (i in 0:(nlevels(lung@active.ident)-1)) {
  cluster <- WhichCells(lung, ident = i)
  name = sprintf("epi_tsne_cluster%s.tiff", i)
  tiff(name, width = 707, height = 500, compression = "lzw")
...
write.csv(lung_cluster.markers, file="epi_MAST_lung_markers.csv")
...
write.csv(top25, file="epi_MAST_lung_cluster_top25_markers.csv")


top5 <- lung_cluster.markers %>% group_by(cluster) %>% top_n(n = 5, wt = avg_log2FC)
write.csv(top5, file="epi_MAST_lung_cluster_top5_markers.csv")
...
write.csv(top10_cluster_tfs, file="epi_MAST_top10_cluster_tfs.csv")
...
write.csv(top10_cluster_cotfs, file="epi_MAST_top10_cluster_cotfs.csv")
...
write.csv(top10_cluster_kinases, file="epi_MAST_top10_cluster_kinases.csv")
...
write.csv(top10_cluster_cds, file="top10_MAST_cluster_cds.csv")
...
write.csv(lung_roc_cluster.markers, file="epi_163k_roc_markers.csv")
...
write.csv(top25_roc, file="epi_163k_roc_cluster_top25_markers.csv")


top5_roc <- lung_roc_cluster.markers %>% group_by(cluster) %>% top_n(n = 5, wt = power)
write.csv(top5_roc, file="epi_163k_roc_cluster_top5_markers.csv")
...
write.csv(top10_roc, file="epi_163k_roc_cluster_top10_markers.csv")
...
# create a new file by merging the TF-list and the ROC enriched genes in each cluster, to identify the TFs, only
...
# export the list of the top 10 TFs based on average expression difference
write.csv(top10_roc_cluster_tfs, file="epi_163k_roc_top10_cluster_tfs.csv")

# create a heatmap with the top 10 ROC enriched TFs
...
# create a new file by merging the coTF-list and the ROC enriched genes in each cluster, to identify the coTFs, only
...
# export the list of the top 10 coTFs based on average expression difference
write.csv(top10_roc_cluster_cotfs, file="epi_163k_roc_top10_cluster_cotfs.csv")
...
# export the list of the top 10 kinases based on average expression difference
write.csv(top10_roc_cluster_kinases, file="epi_163k_roc_top10_cluster_kinases.csv")
...
epi_sct_cells_clusters <- data.frame(lung@active.ident)
write.csv(epi_sct_cells_clusters, file="epi_sct_cells_clusters_sct.csv")

Age <- data.frame(lung@meta.data[["age"]])
experiment <- data.frame(lung@meta.data[["orig.ident"]])
cell_name <- row.names(epi_sct_cells_clusters)
cluster_list <- cbind(cell_name, epi_sct_cells_clusters)
...
write.csv(cluster_list, file="epi_sct_metadata.csv")
...
save.image("epi_163K_after_MAST_and_roc_analysis_data.RData")

#=====================================================================================================
# differential expression analysis of specific clusters

distal_clusters <- c("10", "1", "2", "4", "11", "12", "14", "6", "16")
for (i in 1:length(distal_clusters)){
  a <- FindMarkers(lung, ident.1 = distal_clusters[[i]], ident.2 = distal_clusters[- i], test.use = "MAST", only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.1)
  gene <-row.names(a)
  a <-cbind(gene, a)
  a <-join(a, m, by="gene")
  write.csv(a, file = paste0("distal_cluster", distal_clusters[[i]], "vs_other_distal.csv"))
  rm(a, gene)          
}

proximal_clusters <- c("0", "7", "3", "8", "5", "9")
for (i in 1:length(proximal_clusters)){
  a <- FindMarkers(lung, ident.1 = proximal_clusters[[i]], ident.2 = proximal_clusters[- i], test.use = "MAST", only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.1)
  gene <-row.names(a)
  a <-cbind(gene, a)
  a <-join(a, m, by="gene")
  write.csv(a, file = paste0("proximal_cluster", proximal_clusters[[i]], "vs_other_proximal.csv"))
  rm(a, gene)          
}

ne_clusters <- c("15", "13", "9")
for (i in 1:length(ne_clusters)){
  a <- FindMarkers(lung, ident.1 = ne_clusters[[i]], ident.2 = ne_clusters[- i], test.use = "MAST", only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.1)
  gene <-row.names(a)
  a <-cbind(gene, a)
  a <-join(a, m, by="gene")
  write.csv(a, file = paste0("ne_cluster", ne_clusters[[i]], "vs_other_ne.csv"))
  rm(a, gene)          
}

#===============================================================================================================
DefaultAssay(lung) <-"RNA"
a <- FindMarkers(lung, ident.1 = "2", ident.2 = "10", test.use = "MAST", only.pos = FALSE, min.pct = 0.25,max.cells.per.ident = 428, logfc.threshold = 0.1)
gene <-row.names(a)
a <-cbind(gene, a)
a <-join(a, m, by="gene")
write.csv(a, file = "distal_cl2_vs_cl10.csv")
rm(a, gene)   

DefaultAssay(lung) <-"RNA"
a <- FindMarkers(lung, ident.1 = "3", ident.2 = "9", test.use = "MAST", only.pos = FALSE, min.pct = 0.25,max.cells.per.ident = 428, logfc.threshold = 0.1)
gene <-row.names(a)
a <-cbind(gene, a)
a <-join(a, m, by="gene")
write.csv(a, file = "distal_cl3_vs_cl9.csv")
rm(a, gene)

DefaultAssay(lung) <-"RNA"
a <- FindMarkers(lung, ident.1 = "2", ident.2 = "3", test.use = "MAST", only.pos = FALSE, min.pct = 0.25,max.cells.per.ident = 1332, logfc.threshold = 0.1)
gene <-row.names(a)
a <-cbind(gene, a)
a <-join(a, m, by="gene")
write.csv(a, file = "distal_cl2_vs_cl3.csv")
rm(a, gene)

DefaultAssay(lung) <-"RNA"
a <- FindMarkers(lung, ident.1 = "0", ident.2 = "4", test.use = "MAST", only.pos = FALSE, min.pct = 0.25,max.cells.per.ident = 961, logfc.threshold = 0.1)
gene <-row.names(a)
a <-cbind(gene, a)
a <-join(a, m, by="gene")
write.csv(a, file = "prox_cl0_vs_cl4.csv")
rm(a, gene)


DefaultAssay(lung) <-"RNA"
a <- FindMarkers(lung, ident.1 = c("6", "10"), test.use = "MAST", only.pos = FALSE, min.pct = 0.25,max.cells.per.ident = 428, logfc.threshold = 0.1)
gene <-row.names(a)
a <-cbind(gene, a)
a <-join(a, m, by="gene")
write.csv(a, file = "cl6_cl10_vs_all_other.csv")
rm(a, gene)

#==========================================================================================
#---------------------------------------------------------------------------------------------------------------
levels(lung) <- c("5",  "13", "8",  "3",  "9",  "2",  "10", "1",  "4",  "0",  "6",  "7",  "12", "11", "14")
new_order <- rev(levels(lung))

# analysis based of the top markers according to the avg_log2fc(5 genes)and then difference in the percent of positives (5 genes) 
...
write.csv(e, file="Ext_Fig_4_A_gene_list.csv")
e <- c("EPCAM", "CDH1", "SOX2", "FOXJ1", "CHGA", "ASCL1", "TP63", "KRT5", "SCGB1A1", "SCGB3A2", "SOX9", "FGF20", "HOPX","PDPN", "AQP5", "SFTPC", "ETV5", "MKI67", "PCNA" , e$gene)
e <- e[!duplicated(e)]
#5x20 inches 
DotPlot(lung, assay = "RNA", features = e, scale = TRUE) + RotatedAxis()
known_genes <- c("EPCAM", "CDH1", "SOX2", "FOXJ1", "CHGA", "ASCL1", "TP63", "KRT5", "SCGB1A1", "SCGB3A2", "SOX9", "FGF20", "HOPX","PDPN", "AQP5", "SFTPC", "ETV5", "MKI67", "PCNA")
write.csv(known_genes, file="Ext_Fig_4_A_known_gene_list.csv")

e <- c("EPCAM","NKX2-1", "SOX2", "SCGB3A2", "FOXJ1", "SOX9", "SFTPC", "HIVEP2",  "MSLN", "AGER", 
"PIEZO2", "COL1A2","GRP", "ASCL1", "CALCA", "GHRL", "ACSL1", "RFX6",  "CFC1", "ARX", "NKX2-2", "VSTM2L", "DPP10", "PROX1" )
e <- e[!duplicated(e)]
#5x20 inches 
DotPlot(lung, assay = "RNA", features = e, scale = FALSE) + RotatedAxis()

e <- c("EPCAM","NKX2-1", "SOX2", "SCGB3A2", "SCGB1A1", "FOXJ1", "TP63", "SOX9", "ETV5", "SFTPC", "HIVEP2",  "MSLN", "AGER", 
       "PIEZO2", "COL1A2", "TAGLN", "CLDN5","DPP10", "PROX1", "GRP", "ASCL1", "CALCA", "SST", "GHRL", "ACSL1", "CFC1", "RFX6", "NKX2-2", "VSTM2L", "PCSK1", "ARX")
e <- e[!duplicated(e)]
#5x20 inches 
DotPlot(lung, assay = "RNA", features = e, scale = FALSE) + RotatedAxis()
#===============================================================================================
...
igraph::shortest_paths( g3, from = "2", to = "9" , weights = 1/E(g3)$weight )
