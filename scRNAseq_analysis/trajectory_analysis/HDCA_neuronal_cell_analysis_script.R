...
load("neuro_163k_org_data_with.velo.RData")

neuro
#788 neuronal cells
neuro <- SetIdent(neuro, value= neuro@meta.data[["donor"]])

table(neuro@meta.data[["donor"]])

#keep the donors with more than 24 cells
lung <- subset(neuro, idents= c("donor1", "donor2", "donor3", "donor4", "donor5",  "donor6",
                                "donor7", "donor8", "donor9",   "donor17"))
...
lung.anchors <- FindIntegrationAnchors(object.list = lung.list, normalization.method = "SCT", 
                                       anchor.features = lung.features,dims = 1:24, k.anchor = 20,k.filter = 20, k.score = 20, verbose = TRUE)

rm(lung.list)
lung <- IntegrateData(anchorset = lung.anchors, normalization.method = "SCT", 
                      verbose = TRUE, k.weight = 20)
rm(lung.anchors, lung.features, neuro, stress_den, stress_features, i)


save.image("neuro_batch_integrated.RData")
  
load("neuro_batch_integrated.RData")
...
# set the number of principle components for downstream analyses
pcs=50
...
FeaturePlot(lung, reduction = 'umap', features = "GFAP", pt.size = 2, order = TRUE)
...
# find clusters
lung <- FindClusters(lung, resolution = 2.4)
# run tSNE
...
colors <- DiscretePalette(n_clusters, palette = "polychrome")
...
pdf("neuro_163k_umap3d_clusters.pdf", width = 11.69, height = 8.27, paper = "special")
...
#-------------------------------------------------------------------------------------------------
# reorder the ages
table(lung@meta.data[["age"]])

lung@meta.data[["age"]] <- factor(lung@meta.data[["age"]], levels= c("5w", "5.5w", "6w", "7w", "8w", "8.5w", "14w"))
...
pdf(paste0("neuro_163k_umap3d_",metadata_to_plot,".pdf"), width = 11.69, height = 8.27, paper = "special")
...
pdf("neuro_cells_umap2d_clusters.pdf",width=11.69,height=8.27,paper="special")
neuro_cells_umap2d_clusters <- DimPlot(lung, reduction = "umap2d", cols =  "alphabet2", label = TRUE, pt.size = 3)
plot(neuro_cells_umap2d_clusters)
rm(neuro_cells_umap2d_clusters)
dev.off()

pdf("neuro_cells_umap2d_age.pdf",width=11.69,height=8.27,paper="special")
neuro_cells_umap2d_age <- DimPlot(lung, reduction = "umap2d", cols =  "alphabet2", group.by = "age", pt.size = 3)
plot(neuro_cells_umap2d_age)
rm(neuro_cells_umap2d_age)
dev.off()

pdf("neuro_cells_umap2d_SampleName.pdf",width=11.69,height=8.27,paper="special")
neuro_cells_umap2d_SampleName <- DimPlot(lung, reduction = "umap2d", cols =  "alphabet2", group.by = "SampleName", pt.size = 3)
plot(neuro_cells_umap2d_SampleName)
rm(neuro_cells_umap2d_SampleName)
dev.off()

pdf("neuro_cells_umap2d_donor.pdf",width=11.69,height=8.27,paper="special")
neuro_cells_umap2d_donor <- DimPlot(lung, reduction = "umap2d", cols =  "alphabet2", group.by = "donor", pt.size = 3)
plot(neuro_cells_umap2d_donor)
rm(neuro_cells_umap2d_donor)
dev.off()

pdf("neuro_cells_umap2d_chemistry.pdf",width=11.69,height=8.27,paper="special")
neuro_cells_umap2d_chemistry <- DimPlot(lung, reduction = "umap2d", group.by = "chemistry", pt.size = 3)
plot(neuro_cells_umap2d_chemistry)
rm(neuro_cells_umap2d_chemistry)
dev.off()

pdf("neuro_cells_umap2d_features.pdf",width=11.69,height=8.27,paper="special")
neuro_cells_umap2d_features <- FeaturePlot(lung, reduction = "umap2d",  features = "nFeature_RNA", pt.size = 3)
plot(neuro_cells_umap2d_features)
rm(neuro_cells_umap2d_features)
dev.off()

pdf("neuro_cells_umap2d_percent_mt.pdf",width=11.69,height=8.27,paper="special")
neuro_cells_umap2d_percent_mt <- FeaturePlot(lung, reduction = "umap2d",  features = "percent.MT", pt.size = 3)
plot(neuro_cells_umap2d_percent_mt)
rm(neuro_cells_umap2d_percent_mt)
dev.off()

pdf("neuro_cells_tsne_clusters.pdf",width=11.69,height=8.27,paper="special")
...
pdf("neuro_cells_tsne_age.pdf",width=11.69,height=8.27,paper="special")
neuro_cells_tsne_age <- DimPlot(lung, reduction = "tsne", cols =  "alphabet2", group.by = "age", pt.size = 3)
plot(neuro_cells_tsne_age)
rm(neuro_cells_tsne_age)
dev.off()

pdf("neuro_cells_tsne_SampleName.pdf",width=11.69,height=8.27,paper="special")
neuro_cells_tsne_SampleName <- DimPlot(lung, reduction = "tsne", cols =  "alphabet2", group.by = "SampleName", pt.size = 3)
plot(neuro_cells_tsne_SampleName)
rm(neuro_cells_tsne_SampleName)
dev.off()

pdf("neuro_cells_tsne_donor.pdf",width=11.69,height=8.27,paper="special")
neuro_cells_tsne_donor <- DimPlot(lung, reduction = "tsne", cols =  "alphabet2", group.by = "donor", pt.size = 3)
plot(neuro_cells_tsne_donor)
rm(neuro_cells_tsne_donor)
dev.off()

pdf("neuro_cells_tsne_chemistry.pdf",width=11.69,height=8.27,paper="special")
neuro_cells_tsne_chemistry <- DimPlot(lung, reduction = "tsne", group.by = "chemistry", pt.size = 3)
plot(neuro_cells_tsne_chemistry)
rm(neuro_cells_tsne_chemistry)
dev.off()

pdf("neuro_cells_tsne_percent_mt.pdf",width=11.69,height=8.27,paper="special")
neuro_cells_tsne_percent_mt <- FeaturePlot(lung, reduction = "tsne", features = "percent.MT", pt.size = 3)
plot(neuro_cells_tsne_percent_mt)
rm(neuro_cells_tsne_percent_mt)
dev.off()

pdf("neuro_cells_tsne_features.pdf",width=11.69,height=8.27,paper="special")
neuro_cells_tsne_features <- FeaturePlot(lung, reduction = "tsne", features = "nFeature_RNA", pt.size = 3)
plot(neuro_cells_tsne_features)
rm(neuro_cells_tsne_features)
dev.off()
...
for (i in 0:(nlevels(lung@active.ident)-1)) {
  ...
  name = sprintf("neuro_umap2d_cluster%s.tiff", i)
...
for (i in 0:(nlevels(lung@active.ident)-1)) {
  cluster <- WhichCells(lung, ident = i)
  name = sprintf("neuro_tsne_cluster%s.tiff", i)
  tiff(name, width = 707, height = 500, compression = "lzw")
...
write.csv(lung_cluster.markers, file="neuro_sct_lung_markers.csv")


# filter out insignificant genes
lung_cluster.markers <- lung_cluster.markers[lung_cluster.markers$p_val_adj<0.05, ]
...
write.csv(top25, file="neuro_sct_lung_cluster_top25_markers.csv")


top5 <- lung_cluster.markers %>% group_by(cluster) %>% top_n(n = 5, wt = avg_log2FC)
write.csv(top5, file="neuro_sct_lung_cluster_top5_markers.csv")
...
# plot and inspect informative markers
FeaturePlot(lung, reduction = 'umap2d', features = "EDNRB", pt.size = 2, order = F)
FeaturePlot(lung, reduction = 'umap2d', features = "SOX10", pt.size = 2, order = F)
FeaturePlot(lung, reduction = 'umap2d', features = "FOXD3", pt.size = 2, order = F)
FeaturePlot(lung, reduction = 'umap2d', features = "MPZ", pt.size = 2, order = F)
FeaturePlot(lung, reduction = 'umap2d', features = "SPARC", pt.size = 2, order = F)
FeaturePlot(lung, reduction = 'umap2d', features = "CHGA", pt.size = 2, order = F)
FeaturePlot(lung, reduction = 'umap2d', features = "CHGB", pt.size = 2, order = F)
FeaturePlot(lung, reduction = 'umap2d', features = "DLK1", pt.size = 2, order = F)
FeaturePlot(lung, reduction = 'umap2d', features = "PENK", pt.size = 2, order = F)
FeaturePlot(lung, reduction = 'umap2d', features = "STMN4", pt.size = 2, order = F)
FeaturePlot(lung, reduction = 'umap2d', features = "ISL1", pt.size = 2, order = F)
FeaturePlot(lung, reduction = 'umap2d', features = "ELAVL2", pt.size = 2, order = F)
VlnPlot(lung, features = "STK11")

FeaturePlot(lung, reduction = 'umap2d', features = "NEFM", pt.size = 4, order = F)
#===================================
# run differential expression analyses of distinct clusters
cl5_vs_7 <- FindMarkers(lung, test.use = "MAST", ident.1 = "5", ident.2 = "7", min.diff.pct = 0.1, max.cells.per.ident = 100, only.pos = F, min.pct = 0.25, logfc.threshold = 0.1)
# calculate the percentage difference of the positive cells in the indicated cluster, in comparison to the others
cl5_vs_7$Dpct <- (cl5_vs_7$pct.1 - cl5_vs_7$pct.2)
cl5_vs_7$gene <- row.names(cl5_vs_7)

cl5_vs_7 <-join(cl5_vs_7, m, by="gene")
write.csv(cl5_vs_7, file="neuro_cl5_vs_7_markers.csv")


cl0_vs_1 <- FindMarkers(lung, test.use = "MAST", ident.1 = "0", ident.2 = "1", min.diff.pct = 0.1, max.cells.per.ident = 100, only.pos = F, min.pct = 0.25, logfc.threshold = 0.1)
# calculate the percentage difference of the positive cells in the indicated cluster, in comparison to the others
cl0_vs_1$Dpct <- (cl0_vs_1$pct.1 - cl0_vs_1$pct.2)
cl0_vs_1$gene <- row.names(cl0_vs_1)

cl0_vs_1 <-join(cl0_vs_1, m, by="gene")
write.csv(cl0_vs_1, file="neuro_cl0_vs_1_markers.csv")
...
write.csv(top10_cluster_tfs, file="neuro_MAST_top10_cluster_tfs.csv")
...
write.csv(top10_cluster_cotfs, file="neuro_MAST_top10_cluster_cotfs.csv")
...
write.csv(top10_cluster_kinases, file="neuro_MAST_top10_cluster_kinases.csv")
...
write.csv(top10_cluster_cds, file="neuro_MAST_top10_cluster_cds.csv")
...
write.csv(lung_roc_cluster.markers, file="neuro_163k_roc_markers.csv")
...
write.csv(top25_roc, file="neuro_163k_roc_cluster_top25_markers.csv")


top5_roc <- lung_roc_cluster.markers %>% group_by(cluster) %>% top_n(n = 5, wt = power)
write.csv(top5_roc, file="neuro_163k_roc_cluster_top5_markers.csv")
...
write.csv(top10_roc, file="neuro_163k_roc_cluster_top10_markers.csv")
...
# export the list of the top 10 TFs based on average expression difference
write.csv(top10_roc_cluster_tfs, file="neuro_163k_roc_top10_cluster_tfs.csv")
...
# export the list of the top 10 coTFs based on average expression difference
write.csv(top10_roc_cluster_cotfs, file="neuro_163k_roc_top10_cluster_cotfs.csv")
...
# export the list of the top 10 kinases based on average expression difference
write.csv(top10_roc_cluster_kinases, file="neuro_163k_roc_top10_cluster_kinases.csv")
...
neuro_sct_cells_clusters <- data.frame(lung@active.ident)
write.csv(neuro_sct_cells_clusters, file="neuro_cells_clusters.csv")

Age <- data.frame(lung@meta.data[["age"]])
experiment <- data.frame(lung@meta.data[["orig.ident"]])
cell_name <- row.names(neuro_sct_cells_clusters)
cluster_list <- cbind(cell_name, neuro_sct_cells_clusters)
...
write.csv(cluster_list, file="neuro_metadata.csv")
...
write.csv(abundance_matrix, file="neuro_age_abundance_in_clusters.csv")
...
write.csv(abundance_matrix_percentages, file="neuro_age_abundance_percentages_in_clusters.csv")

save.image("neuro_163K_after_MAST_and_roc_analysis_data.RData")

#=====================================================================================================
# differential expression analysis of specific clusters

distal_clusters <- c("9", "11", "2", "4", "3", "15")
...
#===============================================================================================================
levels(lung) <- c("8", "3", "0", "4", "2", "1", "9", "7", "6", "5")
new_order <- rev(levels(lung))

# analysis based of the top markers according to average expression
top <- lung_cluster.markers %>% group_by(cluster) %>% top_n(n = 5, wt = avg_log2FC)
top <-top[, c(7,6)]
...
e <- c("NRXN1", "ASCL1", # general neuronal
       "FOXD3","SOX10", "PLP1", # schwann precursors from Han-Seop Kim et al., 2017 and Harty and Monk, 2017, Kameneva
        "CDH19", "MPZ", "ERBB2", "ERBB3",  # schwann precursors from Han-Seop Kim et al., 2017
       "PRPH", "ISL1", # neuronal 
        "PCNA", "MKI67" , e$gene
)

e <- e[!duplicated(e)]
#neuronal_top5_markers_logfc_dpct_dotplot
#5x15 inches 
...
for(i in 2*10^-(3) ){
  ...
  g <- g[ g$s != g$p.Var,]
  g$norm.Freq <- g$p.Freq / max(g$p.Freq)
  plot_meta(x = lung,red = "umap2d",feat = "seurat_clusters",frame=T,col = colors,label = F,main = paste0("cutoff=",i), cex=1)
  gpal <- colorRampPalette(c("grey20","black"))(20)
  apply(g,1,function(x){
    lines(as.numeric(x[4:5]),as.numeric(x[6:7]),
          lwd=5*as.numeric(x[8]),
  ...
  points(centroids[,4],centroids[,6],
         bg='white',
         pch=21,cex=2)
  text(centroids[,4],centroids[,6],labels = centroids[,1],cex=.6)
}
