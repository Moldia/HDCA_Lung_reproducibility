...
wd <- "C:/Users/alex/Desktop/cellchat/cellchat_proximal03/"
...
lung <- subset(lung, ident= c("epi_cl1", "epi_cl4", "epi_cl0", "epi_cl6", "epi_cl7", "epi_cl11", "epi_cl12", "epi_cl14", "neuronal", "mes_cl13", "mes_cl5", "mes_cl16", "mes_cl18", "mes_cl10"))

levels(lung) <- c("epi_cl14", "epi_cl11", "epi_cl12", "epi_cl7","epi_cl6", "epi_cl0", "epi_cl4", "epi_cl1","neuronal", "mes_cl10", "mes_cl13", "mes_cl16", "mes_cl5", "mes_cl18")
...
max_value <- data.frame(apply(m[, 2:15], 1, max))
...
# For the chord diagram, CellChat has an independent function netVisual_chord_cell to flexibly visualize the 
# signaling network by adjusting different parameters in the circlize package. For example, we can define a 
# named char vector group to create multiple-group chord diagram, e.g., grouping cell clusters into different cell types.

# Chord diagram
group.cellType <- c(rep("epi", 8), rep("neuronal",1) , rep("mes", 5))# grouping cell clusters into mesenchymal and epithelial
...
# obtain the order of the clusters, to select the ones of interest
rownames(cellchat@netP[["prob"]])
netVisual_bubble(cellchat, sources.use = c(9:14), targets.use = c(1:8), remove.isolate = FALSE, thresh = 0.001)

# show all the significant interactions (L-R pairs) associated with certain signaling pathways
pathways.show.all <- cellchat@netP$pathways
netVisual_bubble(cellchat, sources.use = c(9:14), targets.use = c(1:8), signaling = pathways.show.all, remove.isolate = FALSE,thresh = 0.001)

pdf("pathways_LR_probability_dotplot_mes_target.pdf",width = 9, height = 45, paper = "special")
a<- netVisual_bubble(cellchat, sources.use = c(1:8), targets.use = c(9:14), signaling = pathways.show.all, remove.isolate = FALSE,thresh = 0.001,font.size = 8)
print(a)
rm(a)
dev.off()

pdf("pathways_LR_probability_dotplot_epi_target.pdf",width = 9, height = 45, paper = "special")
a<- netVisual_bubble(cellchat, sources.use = c(9:14), targets.use = c(1:8), signaling = pathways.show.all, remove.isolate = FALSE,thresh = 0.001,font.size = 8)
print(a)
rm(a)
dev.off()

pdf("pathways_LR_probability_dotplot_mes_neunoral.pdf",width = 8, height = 40, paper = "special")
a<- netVisual_bubble(cellchat, sources.use = c(9:14), targets.use = c(9:14), signaling = pathways.show.all, remove.isolate = FALSE,thresh = 0.001,font.size = 8)
print(a)
rm(a)
dev.off()

pdf("pathways_LR_probability_dotplot_epi_neuronal.pdf",width = 15, height = 50, paper = "special")
a<- netVisual_bubble(cellchat, sources.use = c(1:8,14), targets.use = c(1:8,14), signaling = pathways.show.all, remove.isolate = FALSE,thresh = 0.001,font.size = 8)
print(a)
rm(a)
dev.off()
...
a<- netVisual_chord_gene(cellchat, sources.use = c(9:14), targets.use = c(1:8), slot.name = "netP", legend.pos.x = 10)
print(a)
rm(a)
dev.off()

pdf("significant_pathways_from_epi_chordplot.pdf",width = 18, height = 18, paper = "special")
a<- netVisual_chord_gene(cellchat, sources.use = c(1:8), targets.use = c(9:14), slot.name = "netP", legend.pos.x = 10)
print(a)
rm(a)
dev.off()
...
a<- netAnalysis_signalingRole_heatmap(cellchat, pattern = "outgoing",width = 5, height = 15)
print(a)
rm(a)
dev.off()
...
a<- netAnalysis_signalingRole_heatmap(cellchat, pattern = "incoming",width = 5, height = 15)
print(a)
rm(a)
dev.off()
...
a<- netAnalysis_signalingRole_heatmap(cellchat, signaling = pathways.show.all, pattern = "outgoing",width = 5, height = 15)
print(a)
rm(a)
dev.off()

pdf("selected_pathways_incoming_signaling_patterns_heatmap.pdf",width = 10, height = 10, paper = "special")
a<- netAnalysis_signalingRole_heatmap(cellchat, signaling = pathways.show.all, pattern = "incoming",width = 5, height = 15)
print(a)
rm(a)
dev.off()
dev.off()
...
# dot plot
pdf("communication_patterns_outgoing_dotplot.pdf",width = 10, height = 3, paper = "special")
...
saveRDS(cellchat, file = "proximal_cellchat_object.rds")
