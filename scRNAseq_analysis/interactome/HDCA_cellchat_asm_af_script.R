setwd("C:/Users/alex/Desktop/cellchat_asm_af_mature03//")
wd <- "C:/Users/alex/Desktop/cellchat_asm_af_mature03/"
...
lung <- subset(lung, ident= c("mes_cl13", "mes_cl12", "mes_cl16", "mes_cl5"))
levels(lung) <- c("mes_cl13", "mes_cl12", "mes_cl16", "mes_cl5")
...
max_value <- data.frame(apply(m[, 2:5], 1, max))
...
# The function computeAveExpr can help to check the average expression of signaling genes of interest, e.g, "FGF20","BMP2"
...
# obtain the order of the clusters, to select the ones of interest
rownames(cellchat@netP[["prob"]])
netVisual_bubble(cellchat, sources.use = c(3,4), targets.use = c(1,2), remove.isolate = FALSE, thresh = 0.001)

# show all the significant interactions (L-R pairs) associated with certain signaling pathways
pathways.show.all <- cellchat@netP$pathways
netVisual_bubble(cellchat, sources.use = c(3,4), targets.use = c(1,2), signaling = pathways.show.all, remove.isolate = FALSE,thresh = 0.001)

pdf("pathways_LR_probability_dotplot_af_target.pdf",width = 4, height = 8, paper = "special")
a<- netVisual_bubble(cellchat, sources.use = c(1,2), targets.use = c(3,4), signaling = pathways.show.all, remove.isolate = FALSE,thresh = 0.001,font.size = 8)
print(a)
rm(a)
dev.off()

pdf("pathways_LR_probability_dotplot_asm_target.pdf",width = 4, height = 8, paper = "special")
a<- netVisual_bubble(cellchat, sources.use = c(3,4), targets.use = c(1,2), signaling = pathways.show.all, remove.isolate = FALSE,thresh = 0.001,font.size = 8)
print(a)
rm(a)
dev.off()

pdf("pathways_LR_probability_dotplot_af.pdf",width = 3, height = 8, paper = "special")
a<- netVisual_bubble(cellchat, sources.use = c(3,4), targets.use = c(3,4), signaling = pathways.show.all, remove.isolate = FALSE,thresh = 0.001,font.size = 8)
print(a)
rm(a)
dev.off()

pdf("pathways_LR_probability_dotplot_asm.pdf",width = 5, height = 10, paper = "special")
a<- netVisual_bubble(cellchat, sources.use = c(1,2), targets.use = c(1,2), signaling = pathways.show.all, remove.isolate = FALSE,thresh = 0.001,font.size = 8)
print(a)
rm(a)
dev.off()
...
pdf("significant_pathways_from_af_chordplot.pdf",width = 18, height = 18, paper = "special")
a<- netVisual_chord_gene(cellchat, sources.use = c(3,4), targets.use = c(1,2), slot.name = "netP", legend.pos.x = 10)
print(a)
rm(a)
dev.off()

pdf("significant_pathways_from_asm_chordplot.pdf",width = 18, height = 18, paper = "special")
a<- netVisual_chord_gene(cellchat, sources.use = c(1,2), targets.use = c(3,4), slot.name = "netP", legend.pos.x = 10)
print(a)
rm(a)
dev.off()
...
# We also provide another intuitive way to visualize the dominant senders (sources) and receivers (targets) in a 2D space using scatter plot.
...
pdf("communication_patterns_outgoing_heatmap.pdf",width = 6, height =6, paper = "special")
a<- identifyCommunicationPatterns(cellchat, pattern = "outgoing", k = nPatterns, width = 4, height = 8, font.size = 8)
print(a)
rm(a)
dev.off()
...
# dot plot
pdf("communication_patterns_outgoing_dotplot.pdf",width = 6, height = 2, paper = "special")
...
pdf("communication_patterns_incoming_heatmap.pdf",width = 6, height = 6, paper = "special")
a<- identifyCommunicationPatterns(cellchat, pattern = "incoming", k = nPatterns,width = 4, height = 8, font.size = 8)
print(a)
rm(a)
dev.off()
...
# dot plot
pdf("communication_patterns_incoming_dotplot.pdf",width = 6, height = 2, paper = "special")
a<- netAnalysis_dot(cellchat, pattern = "incoming", main.title = "Incoming communication patterns")
print(a)
rm(a)
dev.off()
#================================================================================================
...
saveRDS(cellchat, file = "cellchat_arw_smooth_muscle_arw_fibro_cellchat_object.rds")
