setwd("C:/Users/alex/Desktop/cellchat_neuronal_sm_all03/")

wd <- "C:/Users/alex/Desktop/cellchat_neuronal_sm_all03/"
...
lung1@meta.data[["indiv_cluster"]] <- lung1@meta.data[["indiv_clusters"]]
lung1@meta.data[["indiv_clusters"]] <- NULL

lung1 <- subset(lung1, subset = indiv_cluster != "neuronal")
lung <-lung1
rm(lung1)
...
lung <- subset(lung, ident= c("mes_cl13", "mes_cl12", "mes_cl8", "mes_cl20"))
prox <-lung
rm(lung)

load("neuro_object_after_mast_roc.RData")
DefaultAssay(lung) <- "RNA"
lung@assays[["SCT"]]<- NULL
lung@assays[["integrated"]]<- NULL
lung@reductions[["pca"]] <-NULL
...
lung@meta.data[["indiv_cluster"]] <- as.factor(lung@meta.data[["indiv_cluster"]])
lung <- SetIdent(lung, value="indiv_cluster")

lung <- subset(lung, ident= c("mes_cl13", "mes_cl12", "mes_cl8", "mes_cl20", "neuronal_cl6", "neuronal_cl2", "neuronal_cl3"))

levels(lung) <- c("mes_cl13", "mes_cl12", "mes_cl8", "mes_cl20", 
...
max_value <- data.frame(apply(m[, 2:8], 1, max))
...
levels(cellchat) <- c("mes_cl13", "mes_cl12", "mes_cl8", "mes_cl20", 
                      "neuronal_cl6", "neuronal_cl2", "neuronal_cl3")
...
# that we can compare edge weights between differet networks.
...
vertex.receiver = seq(5,7) # a numeric vector.
...
pathways.show.all
#pathways.show.all <- pathways.show.all[c(c(3:11), c(13:35), c(37:65))] # the missing numbers give error
# check the order of cell identity to set suitable vertex.receiver
levels(cellchat@idents)
vertex.receiver = seq(5,7)
...
# obtain the order of the clusters, to select the ones of interest
rownames(cellchat@netP[["prob"]])
netVisual_bubble(cellchat, sources.use = seq(1,4), targets.use = seq(5,7), remove.isolate = FALSE, thresh = 0.001)

# show all the significant interactions (L-R pairs) associated with certain signaling pathways
pathways.show.all <- cellchat@netP$pathways
netVisual_bubble(cellchat, sources.use = seq(5,7), targets.use = seq(1,4), signaling = pathways.show.all, remove.isolate = FALSE,thresh = 0.001)

pdf("pathways_LR_probability_dotplot_mes_target.pdf",width = 4, height = 12, paper = "special")
a<- netVisual_bubble(cellchat, sources.use = seq(1,4), targets.use = seq(5,7), signaling = pathways.show.all, remove.isolate = FALSE,thresh = 0.001,font.size = 8)
print(a)
rm(a)
dev.off()

pdf("pathways_LR_probability_dotplot_neuro_target.pdf",width = 4, height = 8, paper = "special")
a<- netVisual_bubble(cellchat, sources.use = seq(5,7), targets.use = seq(1,4), signaling = pathways.show.all, remove.isolate = FALSE,thresh = 0.001,font.size = 8)
print(a)
rm(a)
dev.off()

pdf("pathways_LR_probability_dotplot_mes.pdf",width = 5, height = 16, paper = "special")
a<- netVisual_bubble(cellchat, sources.use = seq(1,4), targets.use = seq(1,4), signaling = pathways.show.all, remove.isolate = FALSE,thresh = 0.001,font.size = 8)
print(a)
rm(a)
dev.off()

pdf("pathways_LR_probability_dotplot_neuronal.pdf",width = 4, height = 8, paper = "special")
a<- netVisual_bubble(cellchat, sources.use = seq(5,7), targets.use = seq(5,7), signaling = pathways.show.all, remove.isolate = FALSE,thresh = 0.001,font.size = 8)
print(a)
rm(a)
dev.off()
...
pdf("significant_pathways_from_neuronal_chordplot.pdf",width = 18, height = 18, paper = "special")
a<- netVisual_chord_gene(cellchat, sources.use = seq(5,7), targets.use = seq(1,4), slot.name = "netP", legend.pos.x = 10)
print(a)
rm(a)
dev.off()

pdf("significant_pathways_from_mes_chordplot.pdf",width = 18, height = 18, paper = "special")
a<- netVisual_chord_gene(cellchat, sources.use = seq(1,4), targets.use = seq(5,7), slot.name = "netP", legend.pos.x = 10)
print(a)
rm(a)
dev.off()
...
a<- netAnalysis_signalingRole_network(cellchat, signaling = i, width = 8, height = 2.5, font.size = 10)
print(a)
rm(a)
dev.off()
}
...
a<- netAnalysis_signalingRole_heatmap(cellchat, pattern = "outgoing",width = 3, height = 12)
print(a)
rm(a)
dev.off()
...
a<- netAnalysis_signalingRole_heatmap(cellchat, pattern = "incoming",width = 3, height = 12)
print(a)
rm(a)
dev.off()
...
# Signaling role analysis on the aggregated cell-cell communication network from all signaling pathways
netAnalysis_signalingRole_heatmap(cellchat, pattern = "outgoing")
pdf("outgoing_signaling_patterns_heatmap_blue.pdf",width = 10, height = 10, paper = "special")
a<- netAnalysis_signalingRole_heatmap(cellchat,color.heatmap = "Blues", pattern = "outgoing",width = 3, height = 12)
print(a)
rm(a)
dev.off()

netAnalysis_signalingRole_heatmap(cellchat, pattern = "incoming")
pdf("incoming_signaling_patterns_heatmap_blue.pdf",width = 10, height = 10, paper = "special")
a<- netAnalysis_signalingRole_heatmap(cellchat, color.heatmap = "Blues",pattern = "incoming",width = 3, height = 12)
print(a)
rm(a)
dev.off()
...
a<- netAnalysis_signalingRole_heatmap(cellchat, signaling = pathways.show.all, pattern = "outgoing",width = 3, height = 12)
print(a)
rm(a)
dev.off()

pdf("selected_pathways_incoming_signaling_patterns_heatmap.pdf",width = 10, height = 10, paper = "special")
a<- netAnalysis_signalingRole_heatmap(cellchat, signaling = pathways.show.all, pattern = "incoming",width = 3, height = 12)
print(a)
rm(a)
dev.off()
dev.off()
...
# dot plot
pdf("communication_patterns_outgoing_dotplot.pdf",width = 12, height = 4, paper = "special")
...
saveRDS(cellchat, file = "cellchat_asm_all_neuronal_object.rds")
